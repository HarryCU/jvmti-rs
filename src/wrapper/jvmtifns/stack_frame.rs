...
use crate::{builder::*, errors::*, JVMTIEnv, objects::*};
use crate::sys;
use crate::sys::{jint, jvmtiFrameInfo, jvmtiStackInfo};
...
impl<'a> JVMTIEnv<'a> {
    ...
    pub fn get_all_stack_traces(&self, max_frame_count: jint) -> Result<(Vec<JStackInfo>, jint)> {
        let mut builder: MutAutoDeallocateObjectArrayBuilder<jvmtiStackInfo> = MutAutoDeallocateObjectArrayBuilder::with_size(max_frame_count);
        ...
        jvmti_error_code_to_result(res)?;
        Ok((builder.build(self), thread_count))
    }
